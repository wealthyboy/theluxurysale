<template>
  <div class="">
    ...
    <div class="widget">
      ...
      <div class="collapse show" id="widget-category">
        <div class="widget-body">
          <ul class="cat-list">
            <li
              v-for="sub_category in categories.children"
              :key="sub_category.id"
            >
    ...
    <div
      v-for="filter in categories.attributes"
      :key="filter.id"
      class="widget"
    >
      <h3 class="widget-title">
        <a
          data-toggle="collapse"
          :href="'#widget-body-4' + filter.id"
          role="button"
          aria-expanded="true"
          :aria-controls="'widget-body-4' + filter.id"
          >{{ filter.name }}</a
        >
      </h3>

      <div
        class="collapse cln show"
        :class="filter.children.length"
        :id="'widget-body-4' + filter.id"
      >
        <div class="widget-body">
          <ul class="cat-list">
            <li v-for="children in filter.children" :key="children.id">
              <div class="checkbox">
                <label
                  @change="activateFilter(filter.name, children)"
                  :id="'box' + children"
